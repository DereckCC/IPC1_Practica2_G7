...
public class ProcesoCursos {
    ...
    public ProcesoCursos() {
        cursos = new Cursos[15];
        contador = 0;
    }
        ...
        try {
            ...
            while (linea != null) {
                ...
                for (int z = 0; z < contador; z++) {
                    ...
                    if (cursos[z].getId() == Integer.parseInt(parametros[0])) {
                        flag = true;
                        break;
                    }
                }
                if (flag) {
                    Log.addToEndFile("Logs.txt", " " + new Date().toString() + " El curso con ID: " + Integer.parseInt(parametros[0]) +" Ya fue agregado"+ "\n");
                    linea = br.readLine();      //Leemos una nueva línea del buffer;
                    continue;
                }


                cursos[contador] = new Cursos(Integer.parseInt(parametros[0].trim()), Integer.parseInt(parametros[1].trim()),
                        parametros[2].trim()
                        );
            ...
            System.out.println("Los cursos que no se han podido guardar estan en el log.");
        } catch (Exception e) {
    ...
    public void mostrarArreglo() {
        ...
        for (int i = 0; i < this.contador; i++) {
            System.out.println(this.cursos[i].getId() + "\t\t" + this.cursos[i].getCodigo()
                    + "\t\t" + this.cursos[i].getNombre());
        }
    }
}
