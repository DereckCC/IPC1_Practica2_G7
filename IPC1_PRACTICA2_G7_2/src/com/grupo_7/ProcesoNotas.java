...
public class ProcesoNotas {
   ...
   // public static String[] parametros;
    ...
    public void leerTextoPlano(Alumnos[] alumnos, int contadorAlumnos, Cursos[] cursos, int contadorCursos) throws IOException {
        ...
        try {
            ...
            while (linea != null) {
                ...
                asignarnotas[contador] = new AsignarNotas(Integer.parseInt(parametros[0].trim()), Integer.parseInt(parametros[1].trim()), Double.parseDouble(parametros[2]));
                contador++;
    ...
    public void mostrarArreglo() {
        System.out.println("ID ALUMNO\t\tID CURSO\t\tIDNOTAS");
        for (int i = 0; i < this.contador; i++) {
            System.out.println(this.asignarnotas[i].getIdAlumno() + "\t\t" + this.asignarnotas[i].getIdCurso()+"\t\t" + this.asignarnotas[i].getNota());
        }
    }
}
